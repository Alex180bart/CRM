    ...
    <div className="mx-auto w-full max-w-[86rem] px-5 py-6">
      ...
      <div className="grid gap-4 xl:grid-cols-[1.5fr_1fr]">
                ...
                <TabsContent value="lotes" className="m-0">
                  {campaign.batches.length === 0 ? (
                    <p className="text-muted-foreground py-8 text-center text-xs">
                      Os lotes são criados quando o disparo começa. O tamanho vem da configuração de
                      velocidade.
                    </p>
                  ) : (
        ...
        <div className="flex min-w-0 flex-col gap-4">
          ...
          <Reveal index={5}>
            <div className="bg-card shadow-card rounded-lg p-5">
              <h2 className="font-display mb-3 text-sm font-semibold tracking-tight">Aprovação</h2>
              ...
              <ul className="space-y-2.5">
                {[
                  ...
                  <li key={item.label} className="flex items-start gap-3">
                    <Switch checked={item.on} disabled className="mt-0.5" aria-label={item.label} />
                    <span className="min-w-0 flex-1">
            ...
            <Reveal index={8}>
              <div className="bg-card shadow-card rounded-lg p-5">
                ...
                <div className="bg-muted/60 rounded-lg p-3">
                  ...
                  {template.footerText ? (
                    <p className="text-muted-foreground mt-2 text-[10px]">{template.footerText}</p>
                  ) : null}
      ...
      <Dialog open={approvalOpen} onOpenChange={setApprovalOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              A aprovação libera o disparo para {formatNumber(metrics.eligible)} contatos elegíveis
              e fica registrada na auditoria com autor e horário.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 p-5">
            <dl>
              ...
              />
              <KeyValue label="Custo estimado" value={formatCurrencyCents(metrics.eligible * 74)} />
            </dl>

            <div className="space-y-1.5">
              <label htmlFor="approval-note" className="text-muted-foreground text-xs font-medium">
